...
public class WheelTime {
    ...
    //默认需要年监听
    private boolean isNeedYearListener = true;

    public WheelTime(View view, boolean[] type, int gravity, int textSize, boolean is24Hour) {
        super();
        this.view = view;
        this.type = type;
    ...
    public void setNeedYearListener(boolean needYearListener) {
        isNeedYearListener = needYearListener;
    }
    ...
    private void setSolar(int year, final int month, final int day, int h, int m, int s) {
        ...
        if (is24Hour) {
            wv_hours.setAdapter(new NumericWheelAdapter(0, 23));
            wv_hours.setHalfHour(false);
        } else {
            wv_hours.setAdapter(new NumericWheelAdapter(0, 23));
            wv_hours.setHalfHour(true);
        }
        ...
        if (isNeedYearListener) {
            ...
            wv_year.setOnItemSelectedListener(new OnItemSelectedListener() {
                ...
                public void onItemSelected(int index) {
                    ...
                    currentYear = year_num;
                    int currentMonthItem = wv_month.getCurrentItem();//记录上一次的item位置
                    int currentDayItem = wv_day.getCurrentItem();//记录上一次的item位置
                    NumericWheelAdapter monthAdapter = (NumericWheelAdapter) wv_month.getAdapter();
                    NumericWheelAdapter dayAdapter = (NumericWheelAdapter) wv_day.getAdapter();
                    KLog.d("currentMonthItem: " + currentMonthItem + " currentDayItem: " + currentDayItem);
                    int lastMonth = (int) monthAdapter.getItem(currentMonthItem);
                    KLog.d("currentMonthItem lastMonth: " + lastMonth);
                    int lastDay = (int) dayAdapter.getItem(currentDayItem);
                    KLog.d("currentMonthItem lastDay: " + lastDay);

                    // 判断大小月及是否闰年,用来确定"日"的数据
                    if (year_num != startYear) {

                        //重新设置月份
                        wv_month.setAdapter(new NumericWheelAdapter(1, 12));
                        wv_month.setCurrentItem(--lastMonth);

                        setReDay(year_num, ++lastMonth, 1, 31, list_big, list_little);
                        wv_day.setCurrentItem(--lastDay);
                        ...
                        if (lastMonth < startMonth) {
                            ...
                            if (lastDay <= startDay) {
                                ...
                                wv_day.setCurrentItem(lastDay - startDay);
                            }
                        } else {
                            //如果上次选择的月大于开始的月份
                            wv_month.setCurrentItem(lastMonth - startMonth);
                            setReDay(year_num, month, 1, 31, list_big, list_little);
                            wv_day.setCurrentItem(--lastDay);
                        }
        ...
        if (isNeedYearListener) {
            ...
            wv_month.setOnItemSelectedListener(new OnItemSelectedListener() {
                ...
                public void onItemSelected(int index) {
                    ...
                    NumericWheelAdapter monthAdapter = (NumericWheelAdapter) wv_month.getAdapter();
                    NumericWheelAdapter dayAdapter = (NumericWheelAdapter) wv_day.getAdapter();
                    KLog.d("currentMonthItem:  currentDayItem: " + currentDayItem);
                    int currMonth = (int) monthAdapter.getItem(index);
                    KLog.d("currentMonthItem currMonth: " + currMonth);
                    int lastDay = (int) dayAdapter.getItem(currentDayItem);
                    KLog.d("currentMonthItem lastDay: " + lastDay);
                    if (currentYear == startYear) {
                        if (currMonth < startMonth) {
                            //如果上次选择的月小于开始的月份
                            setReDay(currentYear, currMonth, startDay, 31, list_big, list_little);
                            wv_day.setCurrentItem(0);
                        } else if (currMonth == startMonth) {
                            //如果上次选择的月等于开始的月份
                            setReDay(currentYear, currMonth, startDay, 31, list_big, list_little);
                            if (lastDay <= startDay) {
                                wv_day.setCurrentItem(0);
                            ...
                            //如果上次选择的月大于开始的月份
                            setReDay(currentYear, currMonth, 1, 31, list_big, list_little);
                            wv_day.setCurrentItem(--lastDay);
                        }
                    }else {
                        setReDay(currentYear, currMonth, 1, 31, list_big, list_little);

...
interface ISmartNotifyStatusAidlInterface {
    ...
    //主动通知座椅加热状态，warmStatus为大小为2的数组，依次是左侧座椅加热状态值和右侧座椅加热状态值，值参考Constants.SeatWarm
    void notifySeatWarmStatus(inout int[] warmStatus);

    //主动通知风窗加热状态，warmStatus为大小为2的数组，依次是前挡风玻璃加热状态值和后挡风玻璃加热状态值，值分为0和-1,0为开启状态,-1为关闭状态
    void notifyWindowWarmStatus(inout int[] warmStatus);

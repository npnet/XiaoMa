...
public class PersonalAgeDialog extends BasePersonalInfoDialog implements View.OnClickListener {
    ...
    protected void onBindView(View view) {
        yearWheelView = view.findViewById(R.id.year_view);
        monthWheelView = view.findViewById(R.id.month_view);
        dayWheelView = view.findViewById(R.id.day_view);
    ...
    private void initWheelView() {
        ...
        dayWheelView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(int selectedIndex, String item) {
                if (TextUtils.isEmpty(item)) {
                    int curDay = Calendar.getInstance(Locale.CHINA).get(Calendar.DAY_OF_MONTH);
                    item = String.valueOf(curDay);
                }
                curDay = item;
    ...
    private void modifyDate() {
        ...
        RequestManager.changeUserAge(birthday, new ResultCallback<XMResult<OnlyCode>>() {
            @Override
            public void onSuccess(XMResult<OnlyCode> model) {
                if (model != null && model.isSuccess()) {
                    User user = UserManager.getInstance().getCurrentUser();
                    user.setBirthDayLong(birthday);
                    Calendar birthCalendar = Calendar.getInstance(Locale.CHINA);
                    birthCalendar.setTime(new Date(birthday));
                    int age = endCalendar.get(Calendar.YEAR) - birthCalendar.get(Calendar.YEAR);
                    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.CHINA);
                    user.setAge(String.valueOf(age));
                    user.setBirthDay(dateFormat.format(birthCalendar.getTime()));
                    UserManager.getInstance().notifyUserUpdate(user);
                    onSuccessResult(user.getAge());
                    dismiss();
                } else {
                    onFail(-1, "fail");
                }
            ...
            public void onFailure(int code, String msg) {
                if (!NetworkUtils.isConnected(getContext())) {
                    XMToast.showToast(getContext(), R.string.no_network);
                } else {
                    onFail(code, msg);
                }
                onComplete();

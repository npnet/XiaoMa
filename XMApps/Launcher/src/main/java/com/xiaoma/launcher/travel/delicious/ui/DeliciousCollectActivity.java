...
import com.xiaoma.launcher.travel.delicious.adapter.DeliciousCollectAdapter;
 ...
 * @date 2019/2/3
 */
@PageDescComponent(EventConstants.PageDescribe.deliciousActivityCollect)
public class DeliciousCollectActivity extends BaseActivity {


    public RecyclerView mRvCollect;
    private XmScrollBar mXmScrollBar;
    protected CollectVM mCollectVM;
    //    收藏类型电影Film 酒店Hotel，美食Food，景点Attractions
    public static final String COLLECT_FOOD = "Food";
    private DeliciousCollectAdapter mCollectAdapter;
    private List<SearchStoreBean> searchStoreBeans = new ArrayList<>();
    ...
    private void initView() {
        ...
        tvTip.setText(getString(R.string.no_collection_food));
        mCollectAdapter = new DeliciousCollectAdapter(R.layout.adapter_collect_v1, searchStoreBeans, getString(R.string.btn_navi), getString(R.string.btn_phone));
        ...
        mCollectAdapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
            ...
            public void onLoadMoreRequested() {
                if (!NetworkUtils.isConnected(DeliciousCollectActivity.this)) {
        ...
        mCollectAdapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                if (view.getId() == R.id.collection_linear) {
                    ...
                    mCollectVM.collectItem(mCollectState, mCollectBean.getId(), COLLECT_FOOD, GsonHelper.toJson(mCollectBean));
                    XmAutoTracker.getInstance().onEvent(mCollectBean.getStatus() == BaseCollectVM.HAVE_COLLECT_STATE ? EventConstants.NormalClick.CANCELCOLLECT : EventConstants.NormalClick.COLLECT,       //按钮名称(如播放,音乐列表)
                            ...
                            EventConstants.PagePath.deliciousCollectActivityPath,    //页面路径
                            EventConstants.PageDescribe.deliciousActivityCollect);//页面路径中午意思

                } else if (view.getId() == R.id.tv_action1) {
                    XmAutoTracker.getInstance().onEvent(EventConstants.NormalClick.DELICIOUS_COLLECT_NAVIGATION,       //按钮名称(如播放,音乐列表)
                            GsonUtil.toJson(setDeliciousTracker(searchStoreBeans.get(position))),          //对应id值(如音乐列表对应item id值)
                            EventConstants.PagePath.deliciousCollectActivityPath,    //页面路径
                            EventConstants.PageDescribe.deliciousActivityCollect);//页面路径中午意思
                    ...
                    XmAutoTracker.getInstance().onEvent(EventConstants.NormalClick.DELICIOUS_COLLECT_PHONE,       //按钮名称(如播放,音乐列表)
                            GsonUtil.toJson(setDeliciousTracker(searchStoreBeans.get(position))),          //对应id值(如音乐列表对应item id值)
                            EventConstants.PagePath.deliciousCollectActivityPath,    //页面路径
                            EventConstants.PageDescribe.deliciousActivityCollect);//页面路径中午意思
                    ...
                    if (x1 - x2 > 50) {
                        if (isend) {
                            if (!mRvCollect.canScrollHorizontally(1)) {
                                XMToast.showToast(DeliciousCollectActivity.this, getString(R.string.no_more_data));
                            }
                        }
                    }
                }
    ...
    private void loadMore() {
        ...
        if (pageNo + 1 >= maxPageNum) {
            ...
            pageNo++;
            mCollectVM.fetchCollectFilms(type, COLLECT_FOOD, pageNo);
        }
    }
        ...
        dialog.setContent(s1)
                .setPositiveButton(getString(R.string.travel_left_btn_phone), new XMAutoTrackerEventOnClickListener() {
                    ...
                    public ItemEvent returnPositionEventMsg(View view) {
                        return new ItemEvent(EventConstants.NormalClick.DELICIOUS_COLLECT_PHONE_SURE, GsonUtil.toJson(setDeliciousTracker(searchStoreBean)));
                    }
                ...
                .setNegativeButton(getString(R.string.travel_right_btn), new XMAutoTrackerEventOnClickListener() {
                    @Override
                    public ItemEvent returnPositionEventMsg(View view) {
                        return new ItemEvent(EventConstants.NormalClick.DELICIOUS_COLLECT_PHONE_CANCE, GsonUtil.toJson(setDeliciousTracker(searchStoreBean)));
                    }
...
//            @Override
//            public ItemEvent returnPositionEventMsg(View view) {
//                return new ItemEvent(EventConstants.NormalClick.DELICIOUS_COLLECT_NAVI_SURE,GsonUtil.toJson(setDeliciousTracker(searchStoreBean)));
//            }
//
//            @Override
//            @BusinessOnClick
...
//            @Override
//            public ItemEvent returnPositionEventMsg(View view) {
//                return new ItemEvent(EventConstants.NormalClick.DELICIOUS_COLLECT_NAVI_CANCE,GsonUtil.toJson(setDeliciousTracker(searchStoreBean)));
//            }
//
//            @Override
//            @BusinessOnClick

...
import com.xiaoma.launcher.schedule.utils.CalendarUtil;
...
public class WeekView extends View {
    ...
    private int weekSize = 36;//文字尺寸
    private int weekColor = Color.WHITE;//文字颜色
    private int weekendColor = Color.parseColor("#8a919d");//文字颜色
    ...
    private void initPaint() {
        ...
        mGrayPaint.setColor(weekendColor);
        mGrayPaint.setAntiAlias(true);
        mGrayPaint.setTextSize(weekSize);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < weekArray.length; i++) {
            ...
            if (i == 0 || i == weekArray.length - 1) {
                canvas.drawText(text, startX, startY, mGrayPaint);
            } else {

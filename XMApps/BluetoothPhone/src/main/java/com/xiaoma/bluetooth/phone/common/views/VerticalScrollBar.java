...
public class VerticalScrollBar extends XmSkinView {
     ...
     */
    public static final int FOREGROUND_BAR_MARGIN_LEFT_RIGHT = (BACKGROUND_WIDTH - FOREGROUND_BAR_WIDTH) / 2;
    ...
    public VerticalScrollBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        // 初始化View的固定宽高
        mWidth = mContext.getResources().getDimensionPixelSize(R.dimen.width_vertical_scrollbar);
        mHeight = mContext.getResources().getDimensionPixelSize(R.dimen.height_vertical_scrollbar);
        // 默认隐藏，当有数据加载时会自动修改可见
        setVisibility(GONE);
    }
...
//        shadowBar = mContext.getResources().getDrawable(R.drawable.vertical_scrollbar_shadow);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 滚动条背景
        background.setBounds(0, 0, 12, getHeight());
        if (needChangeAlpha) {
        ...
        // 滚动条前景
        foregroundBar.setBounds(FOREGROUND_BAR_MARGIN_LEFT_RIGHT, mSliderTop,
                BACKGROUND_WIDTH - FOREGROUND_BAR_MARGIN_LEFT_RIGHT, mSliderBottom);
        if (needChangeAlpha) {

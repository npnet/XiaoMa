package com.xiaoma.drivingscore.common.view;
...
public class ChartView extends View {
    ...
    private List<ChartView.Units> mUnitList1 = new ArrayList<>();
    private List<ChartView.Units> mUnitList2 = new ArrayList<>();
...
//            Log.i("wutao", "xInit: " + xInit + "  minXInit: " + minXInit + "  maxXInit: " + maxXInit);
     ...
     * 绘制折线
     */
    private void drawBrokenLine(Canvas canvas, List<ChartView.Units> list, @ColorInt int color) {
        if (list.size() <= 0) {
            return;
        }
        linePaint.setStyle(Paint.Style.STROKE);
     ...
     * 绘制折线对应的点
     */
    private void drawBrokenPoint(Canvas canvas, List<ChartView.Units> list, @ColorInt int color) {
        float dp2 = dpToPx(3);
        float dp4 = dpToPx(4);
        float dp7 = dpToPx(7);
        //绘制节点对应的原点
...
//                    && eventY >= y - dp8 && eventY <= y + rect.height() + dp8
    ...
    public void setValue(List<ChartView.Units> list1, List<ChartView.Units> list2) {
        mUnitList1 = list1;
        mUnitList2 = list2;

...
public class ArtistHandler implements IHandler {
    ...
    public void handle(OnAudioFetchListener listener, List<XMBaseOnlineSection> list) {
        ...
        List<XMArtistInfo> artistInfos = new ArrayList<>();
        for (XMBaseOnlineSection xmBaseOnlineSection : list) {
            ...
            for (BaseQukuItem item : onlineInfoList) {
                ...
                final ArtistInfo albumInfo = (ArtistInfo) item;
                artistInfos.add(new XMArtistInfo(albumInfo));
            }
        }

        listener.onFetchSuccess(artistInfos);
    }

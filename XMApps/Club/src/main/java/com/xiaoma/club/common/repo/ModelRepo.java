...
public abstract class ModelRepo<MODEL> {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    public abstract long[] insertAll(Collection<MODEL> models);

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    public abstract long[] insertAll(MODEL[] models);

    @Delete
    public abstract int delete(MODEL model);

    @Delete
    public abstract int deleteAll(Collection<MODEL> models);

    @Delete
    public abstract int deleteAll(MODEL[] models);
    ...
    public void addObserver(final RepoObserver repoObserver) {
        ...
        ClubRepo.getInstance().getInvalidationTracker().addObserver(realObserver);

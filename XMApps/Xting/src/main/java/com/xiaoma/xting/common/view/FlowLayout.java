...
public class FlowLayout extends RelativeLayout {
    ...
    private static int MAX_LENGTH = 320;
    // 水平间距，单位为dp
    ...
    // 关键字背景
    private int backgroundResource = R.drawable.selector_bg_flowlayout;
    // 关键字水平padding，单位为dp
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (attr == R.styleable.FlowLayoutAttrs_horizontalSpacing) {
                ...
                backgroundResource = typedArray.getResourceId(attr, R.drawable.bg_flow_tv_normal);
            ...
            } else if (attr == R.styleable.FlowLayoutAttrs_singleLine) {
    ...
    public void addViews(List<String> list, final OnItemClickListener onItemClickListener) {
        final String tag = list.get(0);
        for (int i = 1; i < list.size(); i++) {
            final TextView tv = new TextView(getContext());
            final String finalContent = list.get(i);
            // 设置TextView属性
            ...
            tv.setMaxWidth(MAX_LENGTH);
            tv.setMaxEms(12);
            tv.setEllipsize(TextUtils.TruncateAt.END);
            tv.setSingleLine();
            tv.setClickable(true);
//            tv.setBackgroundResource(backgroundResource);
            tv.setBackgroundDrawable(XmSkinManager.getInstance().getDrawable(backgroundResource));
            addView(tv, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT));

            if (onItemClickListener != null) {
                tv.setOnClickListener(new XMAutoTrackerEventOnClickListener() {
                    @Override
                    public ItemEvent returnPositionEventMsg(View view) {
                        return new ItemEvent(finalContent, tag);
                    }

                    @Override
                    @BusinessOnClick
                    public void onClick(View v) {
                        onItemClickListener.onItemClick(finalContent);
                    }
                });
            }
        }

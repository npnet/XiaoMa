...
public abstract class HorizontalRuler extends InnerRuler {
    ...
    public void scrollTo(@Px int x, @Px int y) {
        ...
        if (mRulerCallback != null) {
            int scale = Math.round(mCurrentScale);
            if (mParent.getLastScale() != scale) {
                mParent.setLastScale(scale);
                if (mParent.getMode() == XtingConstants.FMAM.TYPE_FM) {
                    scale *= 100;
                } else {
                    scale = Math.round(scale * mParent.getFactor());
                }
                mRulerCallback.onScaleChanging(scale, byHand);
            }
        }
...
//        scrollTo(scaleToScrollX(mCurrentScale),0);
    ...
    public boolean isScrollingOrTouched() {
        return super.isScrolling() || onTouched;
    }
}
